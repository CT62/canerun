...
export async function getCheapestRate({ shipTo, totalOunces }) {
  ...
  // Freight-class services (e.g. "fedex_3_day_freight") price a single heavy package as
  // LTL freight, which runs into the thousands of dollars — never worth it over ground/parcel.
  const rates = (result.rateResponse?.rates || []).filter(
    (rate) =>
      rate.validationStatus !== 'invalid' &&
      (rate.errorMessages || []).length === 0 &&
      !(rate.serviceCode || '').includes('freight')
  );
  if (rates.length === 0) {
