...
export async function getCheapestRate({ shipTo, totalOunces }) {
  ...
  // "fedex" = a linked FedEx account; "fedex_walleted" = FedEx from ShipStation (no account needed)
  const carrierIds = carriers
    .filter((carrier) => carrier.carrierCode === 'fedex' || carrier.carrierCode === 'fedex_walleted')
    .map((carrier) => carrier.carrierId);
  if (carrierIds.length === 0) {
    throw new Error('No FedEx account is connected in ShipStation.');
  }
